...
function scoreDiagnoses(pred: ClinicalExtraction["diagnoses"], gold: ClinicalExtraction["diagnoses"]): number {
  return setF1(pred, gold, (a, b) => {
    const descScore = tokenSetRatio(a.description, b.description) >= 0.65;
...
function diagnosisMatchesGold(
  ...
  return tokenSetRatio(prediction.description, gold.description) >= 0.65 ||
...
export function evaluateCase(
  ...
  for (const item of prediction.plan) {
    if (!gold.plan.some((goldItem) => tokenSetRatio(item, goldItem) >= 0.65) && !isInTranscript(item, transcript)) {
